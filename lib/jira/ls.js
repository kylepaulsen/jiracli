...
var ls = (function() {
    ...
    var printIssues = function(issues) {
        ...
        for (var x = 0; x < limit; ++x) {
            ...
            console.log('---------------'.grey);
            console.log('Issue'.cyan + '  ' + issue.key);
            console.log('Status'.cyan + '  ' + issue.fields.status.name);
            console.log('Summary'.cyan + '  ' + issue.fields.summary);
            console.log('Priority'.cyan + '  ' + priority);
        }
        if (issues.length) {
            console.log('---------------'.grey);
        } else {
    ...
    var list = function(options) {
        ...
        if (config.lsIgnoreStatuses.length > 0) {
            query += '+AND+status+not+in+("' + config.lsIgnoreStatuses.join('","') + '")';
        }
        if (!all) {
