...
var state = (function() {
    ...
    var warn = function(shouldWarn) {
        shouldWarn = shouldWarn.toLowerCase();
        if (shouldWarn !== 'true' && shouldWarn !== 'false') {
            console.log('jira warn: Expecting true or false.');
            process.exit(1);
        }
        config.warn = shouldWarn === 'true';
        auth.updateConfig();
    };
